...
$request = $_POST['request'];
 ...
 $stmt_request =  $con->prepare("SELECT * FROM lblarticles where transfer_to = '".$request."' and Date(publicate_date) = '".date("Y-m-d")."' ");
  ...
  if($time > $current_time){
   ...
   echo '</div>';
        ...
        ?>
                    ...
                    <a id="font_delete" onclick="return confirm('Are you sure to delete article');"  class="btn btn-clean  btn-sm btn-icon" href="articles-day.php?do=delete&id=<?php  echo $row['id']; ?>"><i class="fa fa-times" aria-hidden="true"></i></a>
...
$stmt_request =  $con->prepare("SELECT * FROM lblarticles where Date(publicate_date) = '".date("Y-m-d")."'  ");
$stmt_request->execute();
$counts = $stmt_request->rowCount(); 
 $rows = $stmt_request->fetchAll();
 ?>
...
$time = $dt->format('H:i:s A');
 ...
 <?php 
    ...
    echo "<h1 style='color:red;'>There's no section data founded in this name</h1>";
  }
