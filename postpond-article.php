...
<select name="section" id="select-section-pt" class="form-control">
...
<div class="input-group input-group-sm input-group-solid">
<select name="d_sort"  id="select-departement-pt" class="form-control">
...
$rows_datetime = $stmt_datetime->fetchAll();
 foreach($rows_datetime as $row_datetime){
    ...
    $publication_time = date('H:i:s', strtotime($get_publication_time));
   ...
   $time_now = date(("H:i:s a"));
            ...
            if($publication_date == $date_now && $publication_time > $time_now || $publication_date > $date_now){

        // if($strip > date("Y-m-d") || $strip == date("Y-m-d") && $stript_time > date("h:i:s")){
